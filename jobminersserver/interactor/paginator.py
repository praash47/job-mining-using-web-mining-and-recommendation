...
class Paginator:
        ...
        for link in links:
            ...
            try:
                if int(link.text) in range(100):
                    logger.info('Paginator found the site has pages, there is integer in range 1-100 in page.')
                    return True
            except: pass
        logger.info('No integer value in range 1-100 found in page. So, Non paginated')
        return False
    ...
    def check_search_step(self, site, links):
        ...
        logger.info(f'Got the search step as {self.search_step}')
    ...
    def get_search_page_url(self, page):
        ...
        logger.info(f'Got url {pre_string + str(page * self.search_step) + post_string} for page {page}.')
