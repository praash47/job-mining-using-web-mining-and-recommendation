...
class Deadline:
    ...
    def __init__(self, tree, xpath=None):
        ...
        self._common_deadline_words = \
            self._parser.get('deadline', 'common_deadline_words').split(',')
        self._common_deadline_words = \
            [word.strip() for word in self._common_deadline_words]
        
        self.deadline = None
        mainlogger.info('Deadline Object Created')
        logger.info('Deadline Object Created')
    ...
    def get_deadline_date(self, html):
        ...
        try:
            ...
            self._check_deadline_words(tags=self._tags_with_date_string, soup=self._soup)
            self._assign_deadline_otherwise()
            self._get_xpath()
            if not self.deadline: raise DeadlineNotFound
            else: 
                mainlogger.info(f'Deadline: {self.deadline}')
                logger.info(f'Deadline: {self.deadline}')
    ...
    def _get_probable_dates(self):
        ...
        in the string. 
        """
        logger.info('Getting probable dates')
        dates = list(
        ...
        if not interested_dates: raise DeadlineNotFound
        # find probable date strings from the interested_dates datetime objects.
        self._probable_date_strings = [self._soup.findAll(text=re.compile(str(date.year))) for date in interested_dates][0]
        logger.info(f'Probable date strings: {self._probable_date_strings}')
    ...
    def _get_tags_with_date_string(self):
        ...
        """
        logger.info('Getting tags with date string')
        for tag in self._soup.findAll():
            if tag.string and not tag.name == 'script' and not tag.name == 'title':  
                ...
                date_to_check = \
                    list(datefinder.find_dates(self._probable_date_strings[0]))[0]
                if self.check_for_nested_tag_label(tag, date_to_check):
                    self._tags_with_date_string.append(tag.name)
        logger.info(f'Tags with date string: {self._tags_with_date_string}')
    ...
    def _assign_deadline_otherwise(self):
        ...
        if not self.deadline and len(self._probable_date_strings) == 1:
            self.deadline = list(datefinder.find_dates(self._probable_date_strings[0]))[0]
    ...
    def _get_xpath(self):
        ...
        def try_get_path(tag):
            ...
            try:
                if list(datefinder.find_dates(str(tag.string)))[0] == self.deadline:
                    try: root = self._tree.getroot().getchildren()
                    ...
                    def assign_el_if_matches(node, tag, **kwargs):
                        ...
                        if node_text == clean_text(tag.string): return node
                        
                        return ''
                    element = assign_el_if_matches(tag, root=root)
                    
                    if type(element) is html.HtmlElement: deadline_xpaths.append(self._tree.getpath(element))
            except Exception:
                try:
        ...
        deadline_xpaths = try_get_path(tags=self._tags_with_date_string, soup=self._soup)

        if not deadline_xpaths: raise DeadlineXpathNotFound

        self.xpath = deadline_xpaths[0]
        logger.info(f'Deadline Xpath: {self.xpath}')
